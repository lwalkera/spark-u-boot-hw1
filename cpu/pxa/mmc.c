...
int
/****************************************************/
mmc_init(int verbose, int bus_width)
/****************************************************/
{
...
#if defined(CONFIG_ADSBITSYXB) || defined(CONFIG_ADSTURBOXB)
	/* power off and on */
	mmc_onoff(0);
	udelay(100000);
	mmc_onoff(1);
	...
	while (retries--) {
		...
		resp = mmc_cmd(SD_CMD_APP_OP_COND, 0x00ff, 0x8000, MMC_CMDAT_R3 | (retries < 2 ? 0 : MMC_CMDAT_INIT));
	...
	if (resp) {
		...
		resp = mmc_cmd(MMC_CMD_SET_RCA, rca, 0, MMC_CMDAT_R1);
		if (IF_TYPE_SD == mmc_dev.if_type)
...
#ifdef CONFIG_PXA27X
	if (IF_TYPE_SD == mmc_dev.if_type && bus_width == 4) {
