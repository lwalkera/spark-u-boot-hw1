...
static void sys_pci_bar_info_get (pci_dev_t devnum, unsigned int bus,
...
int pci_read_config_dword (pci_dev_t dev, int where, unsigned int *val)
{
	...
	addr = BIT( 32 - PCI_DEV(dev) ) | (0x00000F00 & dev) | (where & ~0x03);
	non_prefetch_read (addr, NP_CMD_CONFIGREAD, &retval);

	dprintf("%s: addr: %#x, dev: %#x, where: %#x\n", __FUNCTION__, addr, dev, where);
	dprintf("retval: %#x\n", retval);
...
int pci_read_config_word (pci_dev_t dev, int where, unsigned short *val)
{
	...
	dprintf("%s: addr: %#x, dev: %#x, where: %#x\n", __FUNCTION__, addr, dev, where);
	dprintf("n: %#x, byteEnables: %#x, retval: %#x\n", n, byteEnables, retval);
...
void pci_ixp_init (struct pci_controller *hose)
{
	...
	pci_set_region (hose->regions + 2,
			PCI_IO_BUS,
			PCI_IO_PHY, PCI_IO_SIZE, PCI_REGION_IO);
...
void pci_bus_scan (void)
{
	...
	for (dev = 0; dev < IXP425_PCI_MAX_DEV; dev++) {
		...
		device = PCI_BDF(0, dev, 0);

		/*Check whether a device is present */
		if (pci_device_exists (device) != TRUE) {
		...
		pci_read_config_word (device, PCI_CFG_VENDOR_ID, &data16);

		devices[nDevices].vendor_id = data16;

		pci_read_config_word (device, PCI_CFG_DEVICE_ID, &data16);
		...
		sys_pci_bar_info_get (device, bus, dev, func);
		/*Figure out what INTX# line the card uses */
		pci_read_config_byte (device, PCI_CFG_DEV_INT_PIN, &intPin);
...
#ifdef CONFIG_PCI_SCAN_SHOW
		printf ("%06d    %03d %03d %04d  %08x      %08x\n", nDevices-1,
			devices[nDevices].bus,
			devices[nDevices].device,
			devices[nDevices].func,
			devices[nDevices].device_id,
...
void sys_pci_bar_info_get (pci_dev_t devnum,
			   unsigned int bus,
			   unsigned int dev, unsigned int func)
{
	unsigned int data32;
...
void sys_pci_device_bars_write (void)
{
	...
	for (i = 1; i < nDevices; i++) {
		...
		device = PCI_DEV(devices[i].device);

		pci_write_config_dword (device,
					PCI_CFG_BASE_ADDRESS_0,
					devices[i].bar[0].address);
		pci_write_config_dword (device,
					PCI_CFG_DEV_INT_LINE, devices[i].irq);

		pci_write_config_word (device,
...
pci_dev_t pci_find_devices (struct pci_device_id * ids, int devNo)
{
	...
	for (i = devNo + 1; i < nDevices; i++) {
		...
		dprintf("i: %d, device: %#x, devdidvd: %#x, didvid: %#x\n", i, devices[i].device, devdidvid, didvid);

		pci_read_config_word (devices[i].device, PCI_CFG_VENDOR_ID, &data16);
		dprintf("%#x ", data16);

		pci_read_config_word (devices[i].device, PCI_CFG_DEVICE_ID, &data16);
		dprintf("%#x\n", data16);
...
u8 ixp425_pci_inb(u32 addr)
{
	u32 n, byte_enables, data;
	n = addr % 4;
	byte_enables = (0xf & ~BIT(n)) << PCI_NP_CBE_BESL;
	non_prefetch_read(addr, byte_enables | NP_CMD_IOREAD, &data);

	return data >> (8*n);
}

void ixp425_pci_outb(u32 addr, u8 value)
{
	u32 n, byte_enables, data;
	n = addr % 4;
	byte_enables = (0xf & ~BIT(n)) << PCI_NP_CBE_BESL;
	data = value << (8*n);
	non_prefetch_write(addr, byte_enables | NP_CMD_IOWRITE, data);
}
#endif	/* CONFIG_PCI */
