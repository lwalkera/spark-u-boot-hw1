...
#if defined(__LITTLE_ENDIAN) && defined(CONFIG_IXP425)
inline uchar flash_read_uchar (flash_info_t * info, uint offset)
{
	ulong addr;
	uchar retval;

	addr = (ulong) flash_map (info, 0, offset);
	retval = flash_read8((uchar *) (addr ^ 0x2));
...
ushort flash_read_ushort (flash_info_t * info, flash_sect_t sect, uint offset)
{
	...
	addr = (ulong) flash_map (info, sect, offset);

	retval = flash_read8((uchar *) (addr ^ 0x2));

	addr += info->portwidth; 

	retval |= flash_read8((uchar *) (addr ^ 0x2)) << 8;
...
ulong flash_read_long (flash_info_t * info, flash_sect_t sect, uint offset)
{
	...
	retval = flash_read8((uchar *) (addr ^ 0x2)) << 16;

	addr += info->portwidth; 

	retval |= flash_read8((uchar *) (addr ^ 0x2)) << 24;
...
#ifdef CFG_UNLOCK_WHOLE_AMNESIAC_FLASH_AT_BOOT
static void flash_unlock_amnesiac_flash_at_boot(flash_info_t * info)
{
	unsigned short man_code;
	...
	offset = FLASH_OFFSET_MANUFACTURER_ID;
#if defined(__LITTLE_ENDIAN) && defined(CONFIG_IXP425)
	offset ^= 1;
#endif
	addr = flash_map (info, 0, offset);
	man_code = flash_read16((ushort *) addr);
	flash_unmap (info, 0, offset, addr);

	flash_write_cmd (info, 0, 0, FLASH_CMD_CLEAR_STATUS);
	flash_write_cmd (info, 0, 0, FLASH_CMD_READ_ID);
	offset = FLASH_OFFSET_DEVICE_ID;
#if defined(__LITTLE_ENDIAN) && defined(CONFIG_IXP425)
	offset ^= 1;
#endif
	addr = flash_map (info, 0, offset);
	dev_code = flash_read16((ushort *) addr);
	...
	if (man_code == 0x89) {
		...
		/* check for P30 flash 0x89 == INTEL, 8817, 8818, 8919(sic), 881a, 881b, 891c(sic) are P30 */
		case 0x8817:
		case 0x8818:
		case 0x8919:
		case 0x881a:
		...
		/* check for K3 flash 0x89 == INTEL, 8801 through 8803 are K3 */
		case 0x8801:
		case 0x8802:
		case 0x8803:
			flash_type = "K3";
	...
	if (flash_type) {
		...
		printf("Found %s Flash, unlocking all blocks: ", flash_type);
				
		for (sect=0; sect < info->sector_count; sect++) {
		  printf("%3d\b\b\b", sect);
...
#if defined(__LITTLE_ENDIAN) && defined(CONFIG_IXP425)
#define wp_swizzle(ptr) ((unsigned short *)(((unsigned long) ptr) ^ 2))
					flash_write16(flash_read16(wp_swizzle(src)), wp_swizzle(dst));
#else
...
#ifdef CFG_UNLOCK_WHOLE_AMNESIAC_FLASH_AT_BOOT
		flash_unlock_amnesiac_flash_at_boot(&flash_info[i]);
#endif
