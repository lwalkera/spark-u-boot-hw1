...
int i82365_init (void)
{
	...
	if ((socket.dev = pci_find_devices (supported, 0)) < 0) {
		...
		printf("%s: Controller not found\n", __FUNCTION__);
		return 1;
	}
	debug ("i82365 Device Found!\n");

#ifdef CONFIG_PCMCIA_SLOT_A
	/* function 0 is for the first slot */
	socket.dev = PCI_BDF(PCI_BUS(socket.dev), PCI_DEV(socket.dev), 0);
#endif	
#ifdef CONFIG_PCMCIA_SLOT_B
	/* function 1 is for the second slot */
	socket.dev = PCI_BDF(PCI_BUS(socket.dev), PCI_DEV(socket.dev), 1);
#endif	

#ifdef CONFIG_ADSPORTAL
	/* set base address */
	pci_write_config_dword (socket.dev, PCI_BASE_ADDRESS_0, 0x48000000);
