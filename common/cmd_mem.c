...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if ((flag & CMD_FLAG_REPEAT) == 0) {
		...
		*/
		...
		if (mem_map_get_addr(argv[1], &addr, NULL, NULL, base_address)) {
			printf("Cannot use %s\n", argv[1]);
			return 1;
		}
#else
...
int do_mem_mw ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*/
	...
	if (!mem_map_get_addr(argv[1], &addr, &count, &prot, base_address) &&
	    (prot == MEM_MAP_GUARD_OFF)) {
		count = count / size;
	}
	else {
...
{
	ulong	addr1, addr2, count = 0, ngood;
	int	size;
	int     rcode = 0;
	...
	*/
	...
	if (!mem_map_get_addr(argv[1], &addr1, &count, NULL, base_address)) {
		count = count / size;
	}
	else {
		printf("Cannot use %s\n", argv[1]);
	...
	if (!mem_map_get_addr(argv[2], &addr2, &count2, NULL, base_address)) {
		count2 = count2 / size;
	}
	else {
		printf("Cannot write %s\n", argv[2]);
...
int do_mem_cp ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*/
	...
	if (!mem_map_get_addr(argv[1], &addr, &count, NULL, base_address)) {
		count = count / size;
	}
	else {
		printf("Cannot use %s\n", argv[1]);
	...
	if (!mem_map_get_addr(argv[2], &dest, &count2, &prot, base_address) &&
	    prot == MEM_MAP_GUARD_OFF) {
		count2 = count2 / size;
	}
	else {
