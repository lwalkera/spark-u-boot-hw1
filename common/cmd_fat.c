...
#if defined(CONFIG_CMD_MAP)
	if (mem_map_get_addr(argv[3], &offset, &count, &prot, 0) ||
	    (prot == MEM_MAP_GUARD_ON)) {
...
int do_fat_test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int dev=0;
	int part=1;
	char *ep;
	...
	if (argc < 4) {
		printf ("usage: fattest <interface> <dev[:part]> [path]\n");
		return (0);
	}
	dev = (int)simple_strtoul (argv[2], &ep, 16);
	dev_desc=get_dev(argv[1],dev);
	...
	if (fat_register_device(dev_desc,part)!=0) {
		printf ("\n** Unable to use %s %d:%d for fattest **\n",argv[1],dev,part);
		return 1;
	}

	if (do_fat_read (argv[3], foo, 1, LS_NO) == -1) {
		return 1;
	}
...
U_BOOT_CMD(
	fattest,	4,	1,	do_fat_test,
	"fattest   - test for existence of a path\n",
	"<interface> <dev[:part]> [path]\n"
	"    - return successfully if 'dev' 'interface' 'path' exists\n"
);

int do_fat_env (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	long size;
	char * offset;
	unsigned long count;
	...
	if (argc < 5) {
		printf ("usage: fatenv <interface> <dev[:part]> <filename> <envar>\n");
		return 1;
	}
	dev = (int)simple_strtoul (argv[2], &ep, 16);
	dev_desc=get_dev(argv[1],dev);
	...
	if (fat_register_device(dev_desc,part)!=0) {
		printf ("\n** Unable to use %s %d:%d for fatenv **\n",argv[1],dev,part);
		return 1;
	}


	count = do_fat_read (argv[3], NULL, 0, LS_FSIZE);

	offset = (char *) malloc(count+1);
	if (offset == NULL) {
		printf("Cannot malloc %#x bytes\n", count);
		return 1;
	}

	size = do_fat_read (argv[3], (unsigned char *) offset, count, LS_NO);
...
U_BOOT_CMD(
	fatenv,		5,	1,	do_fat_env,
	"fatenv  - read the contents of a file into an envar\n",
	"<interface> <dev[:part]> <file> <envar>\n"
	"    - put contents of 'dev' 'interface' 'path' into 'envar'\n"
	"      up to FAT_ENVAR_MAX_FILE_SIZE bytes"
);
