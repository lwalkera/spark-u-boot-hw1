...
int _do_setenv (int flag, int argc, char *argv[])
{
	...
	*++env = '\0';
	...
	if (strcmp(argv[1], "bootargs") == 0) {
		void mem_map_parse_bootargs(char *line);
		/* use getenv instead of argv[2] hear because
		   argv[2] isn't always a linear string */
		mem_map_parse_bootargs(getenv(argv[1]));
	}
#endif
	return 0;
}
...
int do_increnv ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	char *str_value;	
	char *ep;
	char new_value[20];
	long num_value;
	long num_increment = 1;
	...
	if ((str_value = getenv(argv[1])) == NULL) {
		printf("No such envar: %s\n", argv[1]);
		return 1;
	}

	if (argc == 3) {
		num_increment = simple_strtol(argv[2], &ep, 0);
		if (*ep != '\0') {
			printf("Invalid increment: %s\n", argv[2]);
			return 1;
		}
	}

	num_value = simple_strtol(str_value, &ep, 0);
	if (*ep != '\0') {
		printf("Envar %s has invalid numeric value: %s\n", argv[1], str_value);
		return 1;
	}

	num_value += num_increment;

	sprintf(new_value, "%d", num_value);

	setenv(argv[1], new_value);
...
int do_askenv ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	timeout_str = getenv("askenv_timeout");
	if (timeout_str) {
		timeout = simple_strtoul (timeout_str, NULL, 10);
		save_timeout = get_cmd_timeout();
...
U_BOOT_CMD(
	increnv, 3, 0,	do_increnv,
	"increnv  - increment environment variables\n",
	"name [increment]\n"
	"    - increment environment variable 'name' by 1 or 'increment'\n"
);
