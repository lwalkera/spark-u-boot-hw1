...
{
	flash_info_t *info;
	ulong bank, addr_first, addr_last, size;
	uchar prot = 0;
...
#if defined(CONFIG_CMD_MAP)
	if (argc == 2 && mem_map_find(argv[1], &addr_first, &size, &prot) == 0) {
		if (prot == MEM_MAP_GUARD_ON) {
			printf("refusing to erase guarded map area\n");
			return 1;
		}
		rcode = flash_sect_erase(addr_first, addr_first + size - 1);
		return rcode;
	}
#endif	  
...
#if defined(CONFIG_CMD_MAP)
	if (argc == 3 && mem_map_find(argv[2], &addr_first, &size, &prot) == 0) {
		rcode = flash_sect_protect (p, addr_first, addr_first + size - 1);
		return rcode;
	}
#endif
...
#if defined(CONFIG_CMD_MAP)
	"protect on <name>\n    - make FLASH sectors in name writable\n"
#endif
