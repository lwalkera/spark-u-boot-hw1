...
#define IDE_SPIN_UP_TIME_OUT 60000 /* 60 sec spin-up timeout */
...
#if defined(CONFIG_IXP425)
	ixp425_pci_outb((ATA_CURR_BASE(dev)+CFG_ATA_PORT_ADDR(port)), val);
#else
...
__ide_inb(int dev, int port)
{
	uchar val;
#if defined(CONFIG_IXP425)
	val = ixp425_pci_inb((ATA_CURR_BASE(dev)+CFG_ATA_PORT_ADDR(port)));
#else
...
#if defined(__PPC__) || defined(CONFIG_PXA_PCMCIA) || defined(CONFIG_SH) || defined(CONFIG_EP93XX)
static void
...
output_data(int dev, ulong *sect_buf, int words)
{
	uchar	*dbuf = (uchar *)sect_buf;
	while (words--) {
		ide_outb(dev, ATA_DATA_EVEN, *(dbuf+0));
		ide_outb(dev, ATA_DATA_ODD, *(dbuf+1));
		ide_outb(dev, ATA_DATA_EVEN, *(dbuf+2));
		ide_outb(dev, ATA_DATA_ODD, *(dbuf+3));
		dbuf += 4;
	}
}
#else	/* ! __PPC__ */
...
static void input_data(int dev, ulong *sect_buf, int words)
{
	uchar	*dbuf = (uchar *)sect_buf;
	while (words--) {
		*(dbuf+0) = ide_inb(dev, ATA_DATA_EVEN);
		*(dbuf+1) = ide_inb(dev, ATA_DATA_ODD);
		*(dbuf+2) = ide_inb(dev, ATA_DATA_EVEN);
		*(dbuf+3) = ide_inb(dev, ATA_DATA_ODD);
		dbuf += 4;
	}
}
...
input_swap_data(int dev, ulong *sect_buf, int words)
{
	uchar	*dbuf = (uchar *)sect_buf;
	while (words--) {
		*(dbuf+1) = ide_inb(dev, ATA_DATA_EVEN);
		*(dbuf+0) = ide_inb(dev, ATA_DATA_ODD);
		*(dbuf+3) = ide_inb(dev, ATA_DATA_EVEN);
		*(dbuf+2) = ide_inb(dev, ATA_DATA_ODD);
		dbuf += 4;
	}
}
#endif
...
#if defined(__PPC__) || defined(CONFIG_PXA_PCMCIA) || defined(CONFIG_EP93XX)
