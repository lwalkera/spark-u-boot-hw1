...
$(obj)include/autoconf.mk: $(obj)include/config.h $(VERSION_FILE)
	...
	$(CPP) -M $(HOST_CFLAGS) $(CPPFLAGS) -MQ $@ include/common.h > $@.dep ; \
...
adssphere_config	\
adssphere_128M_config	\
adssphere_32M_config :	unconfig
	@ >include/config.h
	@[ -z "$(findstring _32M_,$@)" ] || \
		{ echo "#define MEMSIZE 32" >>$(obj)include/config.h ; \
		  echo "... with 32M DRAM" ; \
		}
	@[ -z "$(findstring _128M_,$@)" ] || \
		{ echo "#define MEMSIZE 128" >>$(obj)include/config.h ; \
		  echo "... with 128M DRAM" ; \
		}
	@./mkconfig -a adssphere arm arm920t adssphere NULL ep93xx
...
## XScale Systems
#########################################################################
adsturboxb_config	:	unconfig
	@$(MKCONFIG) $(@:_config=) arm pxa adsturboxb
...
adsbitsyxb_config	\
adsbitsyxb_128M_config :	unconfig
	@ >include/config.h
	@[ -z "$(findstring _128M_,$@)" ] || \
		{ echo "#define MEMSIZE 128" >>$(obj)include/config.h ; \
		  echo "... with 128M DRAM" ; \
		}
	@$(MKCONFIG) -a adsbitsyxb arm pxa adsbitsyxb

adsportal_config	:	unconfig
	@$(MKCONFIG) $(@:_config=) arm ixp adsportal

adsportal_BE_config	:	unconfig
	@$(MKCONFIG) $(@:_config=) arm ixp adsportal_BE

adsvgx_config	\
adsvgx_128M_config	\
adsvgx_256M_config :	unconfig
	@ >include/config.h
	@[ -z "$(findstring _128M_,$@)" ] || \
		{ echo "#define MEMSIZE 128" >>$(obj)include/config.h ; \
		  echo "... with 128M DRAM" ; \
	...
	@[ -z "$(findstring _256M_,$@)" ] || \
		{ echo "#define MEMSIZE 256" >>$(obj)include/config.h ; \
		  echo "... with 256M DRAM" ; \
		}
	@$(MKCONFIG) -a adsvgx arm pxa adsvgx

adsvix_config	\
adsvix_128M_config :	unconfig
	@ >include/config.h
	@[ -z "$(findstring _128M_,$@)" ] || \
		{ echo "#define MEMSIZE 128" >>$(obj)include/config.h ; \
		  echo "... with 128M DRAM" ; \
		}
	@$(MKCONFIG) -a adsvix arm pxa adsvix

cmx270_config: unconfig
	@$(MKCONFIG) $(@:_config=) arm pxa cmx270
...
adsbitsyg5_config	:	unconfig
	@./mkconfig $(@:_config=) arm mx31 adsbitsyg5 
